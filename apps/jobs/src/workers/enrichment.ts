...
export function startEnrichmentWorker() {
	...
	const worker = new Worker<EnrichJobData>(
		QUEUES.ENRICH,
		(job) =>
			withTimeout(() => processEnrichJob(job, meiliBatcher), JOB_TIMEOUT_MS),
		{ connection, concurrency: 32, lockDuration: JOB_TIMEOUT_MS },
	);
...
async function processEnrichJob(
	...
	const type = torrent.releaseData?.type?.toLowerCase() as
		| MediaType
		| undefined;
	...
	const { finalEnrichment, enrichedTorrent } = await db.transaction(
		async (tx) => {
	...
	if (!finalEnrichment || !enrichedTorrent) {
		log.warn(
			{ infoHash },
			"Transaction returned no rows, skipping index update",
		);
		return;
	}
