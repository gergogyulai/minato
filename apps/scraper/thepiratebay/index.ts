...
function parsePublishedAt(dateText: string, timeTitle: string): string | undefined {
...
function parseRows(html: string, baseUrl: string): TorrentInput[] {
  ...
  $("ol#torrents li.list-entry").each((_i, el) => {
    ...
    const leechers = leechersText ? Number.parseInt(leechersText, 10) : undefined;
    ...
    const publishedAt = dateText ? parsePublishedAt(dateText, timeTitle) : undefined;
  ...
  async run({ config, ingest, status, signal }) {
    ...
    if (config.scrapeTop100) {
      for (const cat of ALL_CATEGORIES) {
        ...
        const html = await fetchPage(config.baseUrl, `top100:${cat.id}`, signal);
    ...
    if (config.scrapeTop100_48h) {
      for (const cat of ALL_CATEGORIES) {
        ...
        const html = await fetchPage(config.baseUrl, `top100:48h_${cat.id}`, signal);
        addRows(parseRows(html, config.baseUrl[0] ?? ""));
      }
    }
