...
import { db, eq, sql, user } from "@project-minato/db";
import {
...
} from "@/api/features/setup/contracts";
...
const AVAILABLE_SCRAPERS = [
	{ id: "1337x", name: "1337x", description: "Popular torrent indexer" },
	{
		id: "thepiratebay",
		name: "The Pirate Bay",
		description: "Classic torrent site",
	},
...
export const setupRouter = {
	...
	createAdmin: createAdminContract.handler(async ({ input }) => {
		if (getConfig().setup.setupCompleted) {
			throw new ORPCError("FORBIDDEN", {
				message:
					"Setup has already been completed. Cannot create admin through setup flow.",
			});
		}
	...
	updateScrapers: updateScrapersContract.handler(async ({ input }) => {
		const invalid = input.enabledScrapers.filter(
			(id) => !VALID_SCRAPER_IDS.includes(id),
		);
		if (invalid.length > 0) {
	...
	completeSetup: completeSetupContract.handler(async () => {
		if (!(await hasAdminUser())) {
			throw new ORPCError("BAD_REQUEST", {
				message:
					"Cannot complete setup without creating an admin account first.",
			});
		}
