...
export const scraperRouter = {
  ...
  get: scraperGetContract.handler(async ({ input }) => {
    ...
    if (!row) {
      throw new ORPCError("NOT_FOUND", { message: `Unknown scraper: ${input.id}` });
    }
  ...
  updateConfig: scraperUpdateConfigContract.handler(async ({ input }) => {
    ...
    const allowedKeys = new Set(Object.keys(existing.manifest.defaultConfig ?? {}));
    if (allowedKeys.size > 0) {
      const unknown = Object.keys(input.config).filter((k) => !allowedKeys.has(k));
      if (unknown.length > 0) {
        throw new ORPCError("BAD_REQUEST", {
  ...
  setEnabled: scraperSetEnabledContract.handler(async ({ input }) => {
    ...
    if (!input.enabled) {
      ...
      if (inserted) publishCommand(input.id, { id: inserted.id, command: "stop" });
    }
  ...
  remove: scraperRemoveContract.handler(async ({ input }) => {
    ...
    try {
      ...
      console.warn(`[scraper:remove] revoke api key failed for ${input.id}:`, err);
    }
  ...
  issueCommand: scraperIssueCommandContract.handler(async ({ input, context }) => {
