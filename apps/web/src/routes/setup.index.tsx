...
function ScrapersStep({
	...
	return (
		<div className="space-y-6">
			<p className="text-muted-foreground text-sm leading-relaxed">
				Choose which torrent indexers to enable. You can change these later in
				settings.
			</p>
			...
			<div className="space-y-1">
				{scrapersData?.scrapers.map((scraper: any) => (
					<div
						key={scraper.id}
						className="flex items-center justify-between border-border border-b py-3 last:border-0"
					>
						<div>
							<div className="font-medium text-sm">{scraper.name}</div>
							{scraper.description && (
...
function FlareSolverrStep({
	...
	return (
		<div className="space-y-6">
			<p className="text-muted-foreground text-sm leading-relaxed">
				FlareSolverr helps bypass Cloudflare protection on some torrent sites.
				This is optional — you can skip and configure it later.
			</p>
