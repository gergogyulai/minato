...
Get `MINATO_API_KEY` from **Settings → API Keys** in the dashboard. Create it
with type **Sidecar scraper** and pick a scraper id — the key is bound to that
identity, and your sidecar shows up in the dashboard under it. One key per
sidecar; to rotate, remove the scraper and create a new key.
...
Call once on startup to announce yourself and receive your config. The first
registration creates your scraper's dashboard entry.
...
Send whenever your progress changes, and at least every minute or so — the
dashboard's Live/Offline indicator is driven by when Minato last heard from
you (status posts and the open command stream both count). All fields
optional. Fire-and-forget — don't retry on failure.
...
Open a persistent GET connection and keep reading. Minato pushes an event each time the user triggers a command from the dashboard. The key identifies your scraper — there's no id in the path.

```
GET /api/v1/scraper/commands
X-Minato-Key: <your key>
...
event: command
data: {"id":"550e8400-...","command":"pause","payload":null,"issuedAt":"2026-06-10T12:00:00Z"}
```

Commands: `"pause"`, `"stop"`, `"resume"`.

Acknowledge each command after you've acted on it, so the dashboard shows it
as completed:

```
POST /api/v1/scraper/commands/ack
X-Minato-Key: <your key>

{ "id": "550e8400-..." }
```

When the connection drops, reconnect after a few seconds and send `Last-Event-ID` set to the last event ID you received — Minato will replay any commands you missed while disconnected. Commands expire five minutes after they're issued; a command that was never delivered or acked by then is marked expired and won't be replayed.
